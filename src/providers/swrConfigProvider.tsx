...
const swrMiddleware: Middleware = (useSWRNext: SWRHook) => {
  return (key, fetcher, config) => {
    ...
    const handlerFetcher = async (...args: any) => {
      const cacheData = (cache as Map<string, { data?: any }>).get(String(key))?.data || null;
      if (cacheData && String(key).startsWith("enum-")) return cacheData;
      return fetcher?.(...args);
    };
  ...
  const configValue = useMemo<SWRConfiguration>(
    () => ({
      ...
      onErrorRetry(error, _key, config, revalidate, { retryCount }) {
        ...
        // 404、401 时不重试。
        if (error?.status?.toString().startsWith("4")) return;
